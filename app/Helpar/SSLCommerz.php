...
class SSLCommerz
{
    ...
    {
        try {
            $ssl = sslcommerz_account::first();
            
            if (!$ssl) {
                throw new \Exception('SSLCommerz account not found');
            }
            ...
            Log::error('Error in InitiatePayment: ' . $e->getMessage());
            return ['error' => 'Error in InitiatePayment: ' . $e->getMessage()];
        }
    }
